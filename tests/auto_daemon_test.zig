...
fn appendAccountWithUsage(
    ...
    try fixtures.appendAccount(allocator, reg, email, "", null);
...
test "Scenario: Given free account with only a weekly window when checking current then the free 5h guard does not misfire" {
    ...
    const active_account_key = try fixtures.accountKeyForEmailAlloc(gpa, "free@example.com");
    defer gpa.free(active_account_key);
    try registry.setActiveAccountKey(gpa, &reg, active_account_key);
...
test "Scenario: Given better candidate when auto switch runs then auth and active account move silently" {
    ...
    try appendAccountWithUsage(gpa, &reg, "fresh@example.com", null, null);
    const low_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "low@example.com");
    defer gpa.free(low_account_id);
    try registry.setActiveAccountKey(gpa, &reg, low_account_id);

    const low_auth = try fixtures.authJsonWithEmailPlan(gpa, "low@example.com", "pro");
    ...
    const active_data = try fixtures.readFileAlloc(gpa, active_path);
...
test "Scenario: Given switch-time candidate validation returns non-200 then that candidate is disqualified" {
    ...
    const active_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "active@example.com");
    defer gpa.free(active_account_id);
    const candidate_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "candidate@example.com");
...
test "Scenario: Given daemon api mode and an api-key candidate when auto switching then the candidate stays eligible without usage refresh" {
    ...
    const active_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "active@example.com");
    defer gpa.free(active_account_id);
    const candidate_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "apikey@example.com");
    defer gpa.free(candidate_account_id);
    try registry.setActiveAccountKey(gpa, &reg, active_account_id);
    ...
    const active_auth_data = try fixtures.readFileAlloc(gpa, active_path);
...
test "Scenario: Given healthy active usage when daemon runs then it performs only bounded candidate upkeep" {
    ...
    const candidate_idx = fixtures.findAccountIndexByEmail(&reg, "candidate@example.com") orelse return error.TestExpectedEqual;
...
test "Scenario: Given stale top candidates when daemon switches then it validates them in priority order instead of refreshing all candidates" {
    ...
    const active_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "active@example.com");
    defer gpa.free(active_account_id);
    const first_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "first@example.com");
    defer gpa.free(first_account_id);
    const second_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "second@example.com");
    defer gpa.free(second_account_id);
    const third_account_id = try fixtures.accountKeyForEmailAlloc(gpa, "third@example.com");
    defer gpa.free(third_account_id);
    try registry.setActiveAccountKey(gpa, &reg, active_account_id);

    const first_auth = try fixtures.authJsonWithEmailPlan(gpa, "first@example.com", "pro");
    defer gpa.free(first_auth);
    const second_auth = try fixtures.authJsonWithEmailPlan(gpa, "second@example.com", "pro");
...
test "Scenario: Given automatic switch when writing daemon log then it records source and destination emails" {
    ...
    var reg = fixtures.makeEmptyRegistry();
    defer reg.deinit(gpa);
    try fixtures.appendAccount(gpa, &reg, "from@example.com", "work", null);
    try fixtures.appendAccount(gpa, &reg, "to@example.com", "personal", null);
...
test "Scenario: Given api-backed switch with stale rollout when api later fails then the stale rollout is not assigned to the new active account" {
    ...
    defer reg.deinit(gpa);
    reg.api.usage = true;
    try fixtures.appendAccount(gpa, &reg, "a@example.com", "", null);
    try fixtures.appendAccount(gpa, &reg, "b@example.com", "", null);
    const account_id_a = try fixtures.accountKeyForEmailAlloc(gpa, "a@example.com");
